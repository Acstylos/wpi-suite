...
public class DataStore implements Data {
    ...
    /*
     * Change the following line to rename the database for
     * this instance of WPI Suite running on the server.
     * The default line is:
     * static String WPI_TNG_DB ="WPISuite_TNG_local";
     */
    static String WPI_TNG_DB = "Team1Iteration4.db";
    ...
    //Ignore the warning about the use of type Class
            ...
            public boolean match(Model anObject) {
                try {
                    if (anObject.getProject() == null) {
                        ...
                        return (anObject.getProject().getIdNum().equals(theProject.getIdNum())) && theGetter.invoke(anObjectQueried.cast(anObject)).equals(theGivenValue);
    ...
    public <T> List<Model> retrieveAll(T aSample, Project aProject) {
        ...
        for (Model theModel : allResults) {
            if (theModel.getProject() != null && theModel.getProject().getIdNum().equalsIgnoreCase(aProject.getIdNum())) {
                result.add(theModel);
            }
        }
        System.out.println("retrievedAll: " + result);
    ...
    public List<Model> andRetrieve(final Class anObjectQueried, String[] aFieldNameList, final List<Object> theGivenValueList, final Project aProject) throws WPISuiteException, IllegalArgumentException, IllegalAccessException, InvocationTargetException {
        ...
        for (j = 0; j < theGettersSize; j++) {
            ...
            result = theDB.query(new Predicate<Model>() {
                public boolean match(Model anObject) {
                    try {
                        return anObject.getProject().getIdNum().equals(aProject.getIdNum()) && getBack.invoke(anObjectQueried.cast(anObject)).equals(givenValue);
                    } catch (IllegalArgumentException e) {
                        e.printStackTrace();
                    } catch (IllegalAccessException e) {
                        e.printStackTrace();
    ...
    @SuppressWarnings("rawtypes")
    //Ignore the warning about the use of type Class
    public List<Model> complexRetrieve(final Class andAnObjectQueried, String[] andFieldNameList, final List<Object> andGivenValueList, final Class orAnObjectQueried, String[] orFieldNameList, final List<Object> orGivenValueList, final Project aProject) throws WPISuiteException, IllegalArgumentException, IllegalAccessException, InvocationTargetException {
        List<Model> returnList = new ArrayList<Model>();
        List<Model> bothList = new ArrayList<Model>();
        List<Model> orList = new ArrayList<Model>();
        List<Model> andList = new ArrayList<Model>();
