...
public class ActivityEntityManager implements EntityManager<ActivityModel>{
		 ...
		 * @param s the session
		 * @param id the id number of the Activity to retrieve
		 * 
		 ...
		 */
		@Override
		public ActivityModel[] getEntity(Session s, String id) throws NotFoundException {
			...
			try {
				activities = db.retrieve(ActivityModel.class, "id", intId, s.getProject()).toArray(new ActivityModel[0]);			
		...
		public ActivityModel[] getAll(Session s) {
			...
			return db.retrieveAll(new ActivityModel(), s.getProject()).toArray(new ActivityModel[0]);
		}
		
		/**
		 * Updates the given Activities in the database
		 * @param session the session the Activity to be updated is in
		 * @param content the updated Activity as a Json string	
		 * @return the old Activities prior to updating 
		 * @throws WPISuiteException 
		 * @see edu.wpi.cs.wpisuitetng.modules.EntityManager#update(Session, String) 
		 * */
		@Override
		public ActivityModel update(Session s, String content) throws WPISuiteException {
	        System.out.println("Update Activity: " + content);
	        ActivityModel newActivityModel = ActivityModel.fromJson(content);
			/*
			 * Because of the disconnected objects problem in db4o, we can't just save updatedActivityModel.
			 * We have to get the original ActivityModel from db4o, copy properties from updatedActivityModel,
			 * then save the original ActivityModel again.
			 */
			List<Model> models = db.retrieve(ActivityModel.class, "id", newActivityModel.getId(), s.getProject());
			...
			// copy values to old ActivityModel and fill in our changeset appropriately
		 ...
		 * @throws WPISuiteException if the deletion was not successful
