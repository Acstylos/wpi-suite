...
public class BucketEntityManager implements EntityManager<BucketModel> {
     ...
     *            the session
     * @param id
     *            the id number of the Bucket to retrieve
     * 
    ...
    public BucketModel[] getEntity(Session s, String id)
        ...
        try {
            buckets = db.retrieve(BucketModel.class, "id", intId,
                    s.getProject()).toArray(new BucketModel[0]);
    ...
    public BucketModel[] getAll(Session s) {
        ...
        return db.retrieveAll(new BucketModel(), s.getProject()).toArray(
                new BucketModel[0]);
    }
     ...
     * 
     * @param session
     *            the session the Bucket to be updated is in
     * @param content
     *            the updated Bucket as a Json string
    ...
    public BucketModel update(Session s, String content)
         ...
         * save updatedBucketModel. We have to get the original BucketModel from
         * db4o, copy properties from updatedBucketModel, then save the original
         * BucketModel again.
         */
        ...
        // copy values to old BucketModel and fill in our changeset
        // appropriately
    ...
    /**
     * a method ensures users need to be added!!! Deletes a Bucket from the
     * database
     * 
     * @param s
     *            the session
    ...
    /**
     * a method ensures users need to be added!!! Deletes all Buckets from the
     * database
     * 
     * @param s
     *            the session
