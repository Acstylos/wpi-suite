...
public class TaskPresenter {
    ...
    public TaskPresenter(int id, BucketPresenter bucket, ViewMode viewMode) {
        ...
        view = new TaskView(model, viewMode, this);
    ...
    public void setAssignedUserList() {
        assignedUserList = new ArrayList<Integer>();
    }
    ...
    public void addUsersToView() {
        view.getUserListPanel().removeAllUsers();
        for (User user : allUserArray) {
            if (assignedUserList.contains(user.getIdNum())) {
                view.getUserListPanel().addUserToList(user, true);
            } else {
                view.getUserListPanel().addUserToList(user, false);
            }
        }
    }
    ...
    public void updateFromDatabase() {
        Request request = Network.getInstance().makeRequest(
                "taskmanager/task/" + model.getId(), HttpMethod.GET);
        request.addObserver(new TaskObserver(this));
        request.send();
    ...
    public void setBucket(BucketPresenter bucket) {
        this.bucket = bucket;
        model.setStatus(bucket.getModel().getId());
    }
    ...
    public void removeUserFromAssignedTo(User user) {
        assignedUserList.remove((Object) user.getIdNum());
    }
    ...
    public void addUserToAssignedTo(User user) {
        assignedUserList.add(user.getIdNum());
        view.validateFields();
    }
    ...
    public void setAllowCancelDialogEnabled(boolean enable) {
        allowCancelDialog = enable;
        cancelDialogConfirmed = !enable; // if the dialog is enabled, the
                                         // confirmation of the dialog box is
                                         // opposite
    }
    ...
    public void setIconForMinitaskView() {
        ...
        if (leftInHours == 0) { // On the date it's due
            miniView.setTaskNameLabelIcon(Icons.TASKDUE);
        } else {
            if (leftInHours < -24) { // Overdue
                miniView.setTaskNameLabelIcon(Icons.TASKDUE);
            } else if (leftInHours < 0) { // Nearly due
                miniView.setTaskNameLabelIcon(Icons.TASKNEARDUE);
            } else if (leftInHours < 48) { // In progress
                miniView.setTaskNameLabelIcon(Icons.TASKSTART);
            } else if (dueDate.getMonth() + 1 == 12 && dueDate.getDate() == 25) {
                miniView.setTaskNameLabelIcon(Icons.TASKCHRISTMAS);
            } else if (dueDate.getMonth() + 1 == 4 && dueDate.getDate() == 20) {
                miniView.setTaskNameLabelIcon(Icons.TASKSNOOPDOGG);
            } else if (dueDate.getMonth() + 1 == 10 && dueDate.getDate() == 31) {
                miniView.setTaskNameLabelIcon(Icons.TASKHALLOWEEN);
            } else { // New
                miniView.setTaskNameLabelIcon(Icons.TASKNEW);
            }
        }
    }
    ...
    public void dragDropHistory(int statusBefore, int statusAfter) {
        ...
        activity = user + " has moved this task from "
                + bucket.getWorkflow().idToBucketName(statusBefore) + " to "
                + bucket.getWorkflow().idToBucketName(statusAfter) + " on "
                + dateFormat.format(cal.getTime());
