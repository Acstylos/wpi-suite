...
/**
 * The presenter for the BucketView which sends the view 
 * all of the TaskViews associated with the view.
 * 
...
public class BucketPresenter{
	...
	public void load(){
		...
		final Request request = Network.getInstance().makeRequest("taskmanager/bucketmodel/" + model.getID(), method);
		request.addObserver(new BucketObserver(this, method)); // add an observer to the response
        request.send();
	}
